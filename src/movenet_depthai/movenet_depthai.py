...
_CROP_REGION = namedtuple('_CROP_REGION', ['xmin', 'ymin', 'xmax', 'ymax',
...
class MoveNetDepthAI:
    ...
    def renderNormalized(self, frame: np.ndarray, data=np.ndarray) -> np.ndarray:
        ...
        if data.shape == expectedSize:
            ...
            newlines = [np.array([lines[point] for point in line]) for line in LINES_BODY if
                        lines[line[0]][0] != 0 and lines[line[1]][1] != 0]
    ...
    def renderKeyPoints(self, frame: np.ndarray, kps: np.ndarray) -> None:
        ...
        cv2.rectangle(frame, (self.crop_region.xmin, self.crop_region.ymin),
                      (self.crop_region.xmax, self.crop_region.ymax), (0, 255, 255), 2)

        if data.shape == expectedSize:
            lines = [np.array([data[point] for point in line]) for line in LINES_BODY if
                     data[line[0]][0] != 0 and data[line[1]][1] != 0]
            ...
            for idx, x_y in enumerate(data):
                ...
                if x_y[0] != 0 and x_y[1] != 0:
                    ...
                    raise ValueError(
                        f"Incorrect argument (data) shape. Supplied: {data.shape}, Expected: {expectedSize}")
    ...
    def _determine_crop_region(self, keypoints, scores):
        ...
        if self._torso_visible(scores):
            ...
            center_x = (keypoints[self.KEYPOINT_DICT['left_hip']][0] + keypoints[self.KEYPOINT_DICT['right_hip']][
                0]) // 2
            center_y = (keypoints[self.KEYPOINT_DICT['left_hip']][1] + keypoints[self.KEYPOINT_DICT['right_hip']][
                1]) // 2

            max_torso_yrange, max_torso_xrange, max_body_yrange, max_body_xrange = self._determine_torso_and_body_range(
                keypoints, scores, center_x, center_y)
            ...
            if crop_length_half > max(self.img_w, self.img_h) / 2:
                ...
                return _CROP_REGION(crop_corner[0], crop_corner[1], crop_corner[0] + crop_length,
    ...
    def _find_isp_scale_params(self, size, is_height: bool = True) -> set[int, tuple[int, int]]:
        """
